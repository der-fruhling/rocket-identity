...
impl<'r> FromForm<'r> for Oauth2TokenForm<'r> {
    ...
    fn push_value(ctxt: &mut Self::Context, field: ValueField<'r>) {
        ...
        match &mut ctxt.form {
            Oauth2TokenForm::Init(params) => match key.as_str() {
                "grant_type" => {
                    let v = mem::replace(
                        &mut ctxt.form,
                        match field.value {
                            ...
                            other => {
                                ctxt.errors.push(Error::validation(format!(
                                    "invalid grant type: {other}"
                                )));
                                ctxt.form =
                                    Oauth2TokenForm::Errored(Oauth2Error::UnsupportedGrantType {
            ...
            Oauth2TokenForm::AuthorizationCode {
                ...
                "redirect_uri" => {
                    *redirect_uri = Some(match Url::parse(field.value) {
                        Ok(uri) => uri,
                        Err(e) => {
                            return ctxt.errors.push(Error::validation(format!(
                                "error parsing uri {}: {e}",
                                field.value
                            )));
                        }
            ...
            Oauth2TokenForm::ResourceOwnerPassword {
                ...
                "scope" => {
                    *scope = Some(Box::new(HashSet::from_iter(
                        field.value.split_whitespace().filter(|s| !s.is_empty()),
                    )))
                }
                _ => ctxt.then_error(field.unexpected()),
            },
...
impl From<TokenSignResult> for Oauth2Response<()> {
    fn from(result: TokenSignResult) -> Self {
        Self::new(result.token, "bearer", result.expires)
    }
}
...
pub fn oauth2_token<'r, V: Provider + Oauth2 + Send + Sync + 'static>(
    ...
    Box::pin(async move {
        ...
        let authorization = AuthorizationHeader::from_request(request)
            .await
            .succeeded()
            .and_then(|v| match v {
                AuthorizationHeader::Basic { username, password } => Some((username, password)),
                _ => None,
            });

        let form = if let Some(ty) = request.content_type()
            && ty.is_form()
        {
        ...
        let res = match form {
            ...
            Oauth2TokenForm::AuthorizationCode {
                ...
                if let Some(client_id) = client_id {
                    provider
                        .token_from_authorization_code(client_id, client_secret, code, redirect_uri)
                        .await
                } else {
            ...
            Oauth2TokenForm::ResourceOwnerPassword {
                ...
                if let (Some(client_id), Some(client_secret)) = (client_id, client_secret) {
                    ...
                    provider
                        .token_from_resource_owner_password(
                            client_id,
                            client_secret,
                            username,
                            password,
            ...
            Oauth2TokenForm::ClientCredentials {
                ...
                if let (Some(client_id), Some(client_secret)) = (client_id, client_secret) {
                    let scope: Vec<&str> = scope.unwrap_or_default().into_iter().collect();
                    provider
                        .token_from_client_credentials(client_id, client_secret, scope.as_slice())
                        .await
                } else {
                    Err(Oauth2Error::UnauthorizedClient {
                        error_description: Some("no authorization provided".into()),
...
pub fn oauth2_revoke<'r, V: Provider + Oauth2 + Send + Sync + 'static>(
    ...
    Box::pin(async move {
        ...
        let res = if let Some(client_id) = client_id {
            provider
                .revoke_token(client_id, client_secret, form.token_type_hint, form.token)
                .await
        } else {
...
pub trait Oauth2: Provider {
    ...
    /// This method can be called to generate an authentication code.
    ///
    /// When a client successfully logs in, this will be called to generate an
    /// opaque code that **will be returned to the client.** Their server
    /// will then send this code to the issuing server (yours) that will then
    /// return a token.
    ///
    /// The hope here is that these credentials will not be presented to
    /// the client, and as such may be kept secret more effectively.
    ...
    /// Corresponds to the `authorization_code` grant type.
    ///
    /// This is called by a client server to convert an opaque authorization
    /// code into a useful access token.
    ///
    /// The client secret should only ever be allowed to be [None] if the
    /// client is public (e.g. has explicitly declared that it cannot
    /// maintain confidentiality of its credentials).
    ...
    /// Corresponds to the `resource_owner` grant type.
    ///
    /// This grant is not particularly well-used, but it is nice and simple.
    /// Only "official" clients that you run should ever be able to submit
    /// a grant request like this.
    ...
    /// Corresponds to the `client_credentials` grant type.
    ///
    /// Clients may use request grant to act "as itself." Requests made with
    /// a token obtained via this grant will either not be associated with a
    /// user, or will be associated with a "mannequin" user representing the
    /// client.
    ...
    /// Clients may call this endpoint to revoke a token it possesses.
